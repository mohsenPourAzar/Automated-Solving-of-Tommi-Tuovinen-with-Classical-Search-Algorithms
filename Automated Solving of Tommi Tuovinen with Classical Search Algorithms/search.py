...
class Node:
    ...
    def expand(self, problem,m):
        """List the nodes reachable in one step from this node."""
        return [self.child_node(problem, action)
                
                for action in problem.actions(self.state,m)]
...
def breadth_first_tree_search(problem):
    ...
    while frontier:
        ...
        frontier.extend(node.expand(problem,m))
    return None
...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        ...
        explored.add(node.state)
        m.append(node.action)
        for child in node.expand(problem,m):
...
def recursive_best_first_search(problem, h=None):
    ...
    def RBFS(problem, node, flimit):
        ...
        m.append(node.action)
        successors = node.expand(problem,m)
        if len(successors) == 0:
            return None, np.inf
        for s in successors:
